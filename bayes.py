...
class Bayes_Classifier:
   ...
   def __init__(self):
      ...
      try:
         self.positive = self.load("store_positive")
         self.negative = self.load("store_negative")
         self.positive_number = self.load("store_positive_number")
         self.negative_number = self.load("store_negative_number")
      except IOError:
         self.positive = {}
         self.negative = {}
   ...
   def eval(self):
         """
         Evaluate the native bayes by calculating precision, recall, and f-measure.
         """
         iFileList = []
         positiveFileList = []
         negativeFileList = []
         ...
         for i in range(10):
            correct  = 0 
            classify_positive = 0
            positive_filename = "positive_"+str(i)
            negative_filename = "negative_"+str(i)
            try:
         ...
         precision_result /= 10.0
         recall_result /= 10.0
         fmeasure_result /= 10.0

         
         print precision_result
         print recall_result
         print fmeasure_result
   ...
   def trainForEval(self, train_set, i):
      ...
      for i in train_set:
         ...
         file_list = self.tokenize(file_text)
         ...
         for j in file_list:
            if self.flag == 1:
               ...
               if j not in self.negative.keys():
                  ...
                  self.negative[j] += 1
      self.save(self.positive, "store_positive")
      self.save(self.negative, "store_negative")
      self.save(self.positive_number, "store_positive_number")
      self.save(self.negative_number, "store_negative_number")
